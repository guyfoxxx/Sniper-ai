...
if (text === "/education" || text === BTN.EDUCATION) {
    ...
    if (st.state === "onb_name") {
      ...
      await saveUser(userId, st, env);
      return tgSendMessage(env, chatId, "عالی ✅ حالا شماره تماس را تایپ کن یا با دکمه زیر ارسال کن:", contactKeyboard());
    }
...
function normalizePhone(raw) {
  // Accept typed phone numbers in onboarding. Tries to normalize to a compact digit form.
  // Supports Persian/Arabic digits and optional leading +.
  ...
  // Common Iranian patterns: 09xxxxxxxxx, 9xxxxxxxxx, +989xxxxxxxxx, 989xxxxxxxxx
  ...
  // Basic sanity: 10-15 digits (or 11 for Iran mobile)
